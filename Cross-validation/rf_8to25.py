...
cu_rf_params = {                 
'n_estimators': 999,    # number of trees in forest       
'max_depth': 999,         # maximum depth of tree      
'n_bins': 15,           # Number of bins used to discretize continuous features             
'n_streams': 8 }   
...
for train_indices, val_indices in k_fold.split(cv_indices):
	...
	# with open('validationlabels','wb') as f: pickle.dump(new_predictions, f)
	# with open('truevalidationlabels','wb') as f: pickle.dump(validationlab, f)
...
# plot_confusion_matrix(loaded_model, test_features, test_labels)
